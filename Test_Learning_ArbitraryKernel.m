...
%         model3 = Learning_LS_Discrete( Seqs1(1:i*nNum), model3 );
        ...
        for u = 1:D
            for v = 1:D
                ...
                plot(model2.dt*(0:model2.M-1), model2.g * model2.A(v,:,u)'./model2.dt, '-')
                %plot(model3.h*(0:(size(model3.A,2)-1)), model3.A(v,:,u), '-')
                hold off
                axis tight
                legend('Real', 'MLE-Basis')%, 'MLE-ODE', 'LS')%, 'LS2')
